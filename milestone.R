...
# index <- sample(1:length(news), ceiling(.1*length(news))); newTrain <- news[index]
...
clean <- function(corpus){
    ...
    cleaned <- stri_replace_all(cleaned, regex = "'ve", replacement = " have")
    cleaned <- stri_replace_all(cleaned, regex = "'re", replacement = " are")
    cleaned <- stri_replace_all(cleaned, regex = "n't", replacement = " not")
    cleaned <- stri_replace_all(cleaned, regex = "'ll", replacement = " will")
    cleaned <- stri_replace_all(cleaned, regex = "'d", replacement = " would")
    cleaned <- stri_replace_all(cleaned, regex = "\\b(i'm|im)\\b", replacement = "i am")
    cleaned <- stri_replace_all(cleaned, regex = "\\b(he|she|it|how|that|there|what|when|who|why|where)'s", replacement = "$1 is")
    ...
    cleaned <- stri_replace_all(cleaned, regex = " ?[<>0O%]?[:;=8]([-o*']+)?[()dp/ocs]+|[()dp/}{#|c]+([-o*']+)?[:;=8][<>]?|<+3+|<+/+3+|[-o0><^][_.]+[-o0><^] | ?[<>0O%]?[:;=8]([-o*']+)?[()dp/ocs]+|[()dp/}{#|c]+([-o*']+)?[:;=8][<>]?|<+3+|<+/+3+|[-o0><^][_.]+[-o0><^]$", replacement = " <emoticon> ")
    # find and replace email data
    cleaned <- stri_replace_all(cleaned, regex = " ?([\\w\\d_.-]+)@([\\w\\d.-]+)\\.([\\w.]{2,6}) ?", replacement = " <email> ")
    # find and replace url
    cleaned <- stri_replace_all(cleaned, regex =" ?(https:/+)?(([\\w\\d]+)\\.){1,4}(tv|com|org|edu|gov|me|net|info)[/\\w-!=?@_]* ?", replacement = " <url> ", simplify = T, omit_no_match = T)
    ...
    cleaned <- stri_replace_all(cleaned, regex = "(?<=[!/,.:;?#])[!,.:;#?]+|@ ?\\w+\\b|'+", replacement = "")
    ...
    cleaned <- stri_replace_all(cleaned, regex ="\\b[\\d]+([-,.\\d]+)?(th|st|nd|rd)?\\b", replacement= " <num> ")
...
# build ngram.offset
## system.time(ngram.offset(cleaned))


# clean data
cleaned <<- wrap(clean(c(twitter.all, news.all, blogs.all)))
...
interpolate <- function(input){
    j <- length(phrase)
    fourN <- fourgram[J(phrase[j-3], phrase[j-2], phrase[j-1], phrase[j]), nomatch=0]$N
    triN <- trigram[J(phrase[j-2], phrase[j-1], phrase[j]), nomatch=0]$N
    biN <- bigram[phrase[j-1], phrase[j], nomatch=0]$N
    uniN <- unigram[phrase[j], nomatch=0]$N
